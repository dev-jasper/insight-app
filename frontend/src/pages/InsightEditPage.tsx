...
function getApiError(err: unknown): { form: string; fields: FieldErrors } {
...
export default function InsightEditPage() {
    ...
    useEffect(() => {
        ...
        getInsight(insightId)
            ...
            .catch((err) => {
                if (!mounted) return;
                setFormError(getApiError(err).form);
            })
            .finally(() => {
                if (!mounted) return;
                setLoading(false);
    ...
    return (
        <main style={{ padding: 24, fontFamily: "system-ui", maxWidth: 720, margin: "0 auto" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
                ...
                <Link to={initial ? `/insights/${initial.id}` : "/insights"}>← Back</Link>
            </div>
                ...
                <form onSubmit={onSubmit} style={{ marginTop: 16, display: "grid", gap: 14 }}>
                    ...
                    <div>
                        ...
                        {(fieldErrors.title || titleErr) && (
                            <div style={{ color: "crimson", marginTop: 4 }}>{fieldErrors.title || titleErr}</div>
                        )}
                    </div>
                    ...
                    <div>
                        ...
                        {(fieldErrors.category || categoryErr) && (
                            <div style={{ color: "crimson", marginTop: 4 }}>{fieldErrors.category || categoryErr}</div>
                        )}
                    </div>
                    ...
                    <div>
                        ...
                        {(fieldErrors.body || bodyErr) && (
                            <div style={{ color: "crimson", marginTop: 4 }}>{fieldErrors.body || bodyErr}</div>
                        )}
                    </div>
                    ...
                    <div>
                        ...
                        {(fieldErrors.tags || tagsErr) && (
                            <div style={{ color: "crimson", marginTop: 4 }}>{fieldErrors.tags || tagsErr}</div>
                        )}
                    </div>
                    ...
                    <div style={{ display: "flex", gap: 10 }}>
                        ...
                        <button type="button" onClick={() => nav(`/insights/${initial.id}`)} disabled={submitting}>
                            Cancel
                        </button>
                    </div>
                </form>
