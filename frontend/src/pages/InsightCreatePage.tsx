...
export default function InsightCreatePage() {
    ...
    async function onSubmit(e: React.FormEvent) {
        ...
        try {
            ...
            setFormError(parsed.message || "Request failed. Please try again.");
    ...
    return (
        <PageShell title="Create Insight" subtitle="Write a new insight and tag it for analytics">
            <div style={styles.card}>
                {formError && (
                    <div style={styles.alert}>
                        <div style={{ fontWeight: 900, marginBottom: 4 }}>Create failed</div>
                ...
                <form onSubmit={onSubmit} style={styles.form}>
                    ...
                    <div style={styles.field}>
                        ...
                        />
                        <div style={styles.hint}>{title.trim().length}/{TITLE_MAX}</div>
                        {(fieldErrors.title || titleErr) && <div style={styles.fieldError}>{fieldErrors.title || titleErr}</div>}
                    </div>
                    ...
                    <div style={styles.field}>
                        ...
                        {(fieldErrors.category || categoryErr) && <div style={styles.fieldError}>{fieldErrors.category || categoryErr}</div>}
                    </div>
                    ...
                    <div style={styles.field}>
                        ...
                        />
                        {(fieldErrors.body || bodyErr) && <div style={styles.fieldError}>{fieldErrors.body || bodyErr}</div>}
                    </div>
                    ...
                    <div style={styles.field}>
                        ...
                        <div style={styles.tagRow}>
                            ...
                            />
                            <button type="button" onClick={addTag} disabled={!tagInput.trim() || tags.length >= TAG_MAX} style={styles.secondaryBtn}>
                                Add
                            </button>
                        </div>
                        ...
                        <div style={styles.pills}>
                            {tags.map((t) => (
                                <button key={t} type="button" onClick={() => removeTag(t)} style={styles.pill} title="Remove tag">
                                    #{t} <span style={{ opacity: 0.85 }}>✕</span>
                                </button>
                            ))}
                        </div>

                        {(fieldErrors.tags || tagsErr) && <div style={styles.fieldError}>{fieldErrors.tags || tagsErr}</div>}
                    </div>
                    ...
                    <div style={styles.actions}>
                        ...
                        <button type="button" onClick={() => nav("/insights")} disabled={submitting} style={styles.ghostBtn}>
                            Cancel
                        </button>
                    </div>
                </form>
